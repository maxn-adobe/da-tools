...
/** Which bulk operation is reporting progress (drives an n/m counter in the callers). */
...
export function useDaDocumentActions<T extends RowResult>(
  ...
  // Patch by row `id`; look up outcomes by `path` (a path can map to >1 row in the Generate tab).
  async function runBulkStage(
    ...
    try {
      ...
      // Whole-op failure (token lost, unexpected throw) → today's per-row fan-out (full per-row UI).
